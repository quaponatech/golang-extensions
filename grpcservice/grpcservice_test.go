...
func TestSuiteGRPCService(t *testing.T) {
	var okName = "Test GRPC Service"

	t.Run("SetupWrong", func(t *testing.T) {
		var (
			wrongName     = ""
			wrongChannel  chan bool
			wrongServer   *grpcservice.GRPCServer
		...
		var (
			...
			okServer  = grpcservice.NewGRPCServer(false, "", "", mainPort+portCounter)
		)

		t.Run("WrongPrefix", func(t *testing.T) {
			tempService := new(grpcservice.GRPCService)
		...
		t.Run("WrongServer", func(t *testing.T) {
			...
			test.AssertThat(t, err, "GRPC server not initialized", "streq")
		})
	...
	t.Run("SetupOK", func(t *testing.T) {
		...
		t.Run("StopSucceedsOnNotRunningService", func(t *testing.T) {
			...
			err := tempService.Setup(okName,
				grpcservice.NewGRPCServer(false, "", "", mainPort+portCounter),
	...
	t.Run("SetupFailsOnAlreadyStoppedGRPCServer", func(t *testing.T) {
		// Setup
		tempService := new(grpcservice.GRPCService)
		portCounter++
		...
		err = tempService.Setup(t.Name(),
			...
			server.NewLogger(t.Name(), "", "",
				...
				make(chan string), make(chan string), 0),
			make(chan bool))
		test.AssertThat(t, err, fmt.Errorf("GRPC server not initialized"))
	...
	t.Run("StartServingFailsOnAlreadyStoppedGRPCServer", func(t *testing.T) {
		// Setup
		tempService := new(grpcservice.GRPCService)
		portCounter++
		tempServer := grpcservice.NewGRPCServer(false, "", "", mainPort+portCounter)
	...
	t.Run("AlreadyStartedServingFailsOnAlreadyStoppedGRPCServer", func(t *testing.T) {
